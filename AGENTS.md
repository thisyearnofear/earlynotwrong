...
> Last updated: 2026-08-15. Live commit on `nuncio-vultr`: `a88c6c4c` (both-sides incident fix). Live state: one open position (Typhoon Dolphin YES, 103.26 TST), bankroll ~874.78 TST (806.52 + 68.26 recovered from the stopped NO leg). **Inference heads-up:** both free sources are exhausted — the Vercel AI Gateway's promo credit ran dry (permanent until topped up; breaker skips it), and the OpenRouter account hit its free-model daily quota (50 req/day; resets 00:00 UTC). Cycles run 0 estimates until then; the vol baseline + forecast cache keep everything else live.
