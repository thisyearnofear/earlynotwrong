...
> Last updated: 2026-08-18. Live commit on `nuncio-vultr`: `f7f1dad5` (keyless Firecrawl + Parallel briefing ladder). **Agent trading live on 3 positions** (~271 TST deployed, 786 TST bankroll): Typhoon Dolphin YES (fc 0.95 @ 0.317), Trump Senate nominations (fc 0.95), Chess Wikipedia pageviews (fc 0.85 @ 0.666 — entered by the new hf-qwen tier). Estimates recovered to 14/15 per cycle after both quota-limited sources ran dry; briefings back online via keyless search rungs. The 4-tier evidence-validation stack (fact authorities, plausibility filter, two-source corroboration, adversarial pre-entry verification) is shipped and pending deploy.
