...
> Last updated: 2026-08-19 (evening). Live commit on `nuncio-vultr`: `754448d0` (b-ai ladder swap). **Agent trading live on 3 Delphi positions** (~263 TST deployed; 931.95 TST bankroll recovered after the stale-subgraph fix freed the settled Chess market): UAP records (fc 0.85 @ 0.56, resolves 08-21), Treasury 10Y (fc 0.50 @ 0.32, resolves 08-20), Botafogo 4+ goals (fc 0.65 @ 0.28, resolves 08-20). Inference recovered from a full free-ladder collapse (hf-qwen 404-retired, OrcaRouter qwen quota-dead) via the new b-ai DeepSeek-V4-Flash rung. The all-forecasts calibration ledger (`estimates.jsonl` → `forecasts-all.jsonl`) is live, so calibration is no longer selection-biased to traded markets.
