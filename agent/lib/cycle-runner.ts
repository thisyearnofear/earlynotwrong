...
import { recordAgentEntry, recordAgentExit } from "./self-analysis.js";
...
export async function harvestForBnb(): Promise<void> {
  ...
  if (primary.success) {
    ...
    const harvestExitPriceUsd = target.entryPriceUsd > 0 ? (targetValueUsd * target.entryPriceUsd) / target.amountUsd : 0;
    recordAgentExit({
      symbol: target.symbol,
      priceUsd: harvestExitPriceUsd,
  ...
  if (toUsdc.success) {
    ...
    if (usdcUsd > 0.5) {
      ...
      if (usdcToBnb.success) {
        ...
        const hopExitPriceUsd = target.entryPriceUsd > 0 ? (targetValueUsd * target.entryPriceUsd) / target.amountUsd : 0;
        recordAgentExit({
          symbol: target.symbol,
          priceUsd: hopExitPriceUsd,
  ...
  if (tinyTest.success) {
    ...
    if (retry.success) {
      ...
      const probeExitPriceUsd = target.entryPriceUsd > 0 ? (targetValueUsd * target.entryPriceUsd) / target.amountUsd : 0;
      recordAgentExit({
        symbol: target.symbol,
        priceUsd: probeExitPriceUsd,
        amountUsd: 1.5,
