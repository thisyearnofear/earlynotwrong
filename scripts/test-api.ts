...
async function testHeliusApiDirect(address: string) {
  ...
  try {
    ...
    if (data.length > 0) {
      ...
      console.log(
        `  Timestamp: ${new Date(data[0].timestamp * 1000).toISOString()}`,
      );
      // Show a sample transaction structure
      if (data[0].tokenTransfers?.length > 0) {
        console.log(
          `  Sample tokenTransfers:`,
          JSON.stringify(data[0].tokenTransfers.slice(0, 2), null, 2),
        );
      }
    }
    return data;
...
async function testAlchemyApiDirect(address: string) {
  ...
  try {
    ...
    console.log(
      `  ✅ Alchemy returned ${transfers.length} transfers (from address)`,
    );
    ...
    console.log(
      `  ✅ Alchemy returned ${incomingTransfers.length} incoming transfers`,
    );

    if (transfers.length > 0) {
      console.log(
        `  Sample transfer: ${transfers[0].asset} - ${transfers[0].value}`,
      );
    }
...
async function testTransactionsApi(wallet: (typeof SHOWCASE_WALLETS)[0]) {
  ...
  try {
    ...
    if (data.quality) {
      ...
      console.log(
        `     - Filtered (invalid/LP): ${data.quality.invalidFiltered}`,
      );
      console.log(
        `     - Data completeness: Symbol ${data.quality.dataCompleteness.symbolRate}%, Price ${data.quality.dataCompleteness.priceRate}%, Amount ${data.quality.dataCompleteness.amountRate}%`,
      );
    ...
    if (data.transactions?.length > 0) {
      console.log(
        `  Sample: ${data.transactions[0].tokenSymbol || "Unknown"} - $${data.transactions[0].valueUsd.toFixed(2)}`,
      );
    }
...
async function main() {
  ...
  console.log(
    `  ALCHEMY_API_KEY: ${process.env.ALCHEMY_API_KEY ? "✅ Set" : "❌ Missing"}`,
  );
  ...
  const deployerWallet = SHOWCASE_WALLETS.find(
    (w) => w.id === "deployer-base",
  )!;
  ...
  try {
    ...
    if (!healthCheck.ok) {
      ...
      console.log(
        "   Direct API tests above show your API keys are working correctly.\n",
      );
      return;
    }
  } catch {
