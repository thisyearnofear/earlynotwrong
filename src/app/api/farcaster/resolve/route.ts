...
export async function POST(request: NextRequest) {
    try {
        ...
        const farcasterProfile = profiles.find((p: any) => p.platform === 'farcaster');
        const ensProfile = profiles.find((p: any) => p.platform === 'ens');
        const basenamesProfile = profiles.find((p: any) => p.platform === 'basenames');
        const primaryProfile = farcasterProfile || ensProfile || basenamesProfile || profiles[0];
