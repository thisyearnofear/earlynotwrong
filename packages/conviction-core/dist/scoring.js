...
// Defaults
// =============================================================================
export const DEFAULT_BEHAVIORAL_WEIGHTS = {
    winRate: 0.25,
    upsideCapture: 0.35,
...
export const DEFAULT_ARCHETYPE_THRESHOLDS = {
...
export function groupEntriesIntoPositions(entries) {
    const map = new Map();
    ...
    return Array.from(map.values()).filter((p) => p.entries.length > 0 && p.totalInvested > 0);
}
// =============================================================================
// Patience tax
// =============================================================================
 ...
 */
export function calculatePatienceTax(exitPrice, positionSize, priceHistory, windowDays = 90, fromTimestamp) {
    ...
    for (const point of priceHistory) {
        if (point.timestamp < exitTs || point.timestamp > endTs)
            continue;
...
export function analyzePosition(input) {
    ...
    const realizedPnLPercent = position.totalInvested > 0 ? (realizedPnL / position.totalInvested) * 100 : 0;
    ...
    const holdingPeriodDays = position.exits.length > 0
    ...
    if (position.entries.length > 1) {
        for (let j = 1; j < position.entries.length; j++) {
            const gapDays = (position.entries[j].timestamp - position.entries[j - 1].timestamp) /
                (24 * 60 * 60 * 1000);
            if (gapDays > 1) {
                hasReEntry = true;
                break;
    ...
    if (position.exits.length > 0 && priceHistory && priceHistory.length > 0) {
        ...
        patienceTaxData = calculatePatienceTax(lastExitEntry.priceUsd, position.totalRealized, priceHistory, patienceTaxWindowDays, lastExitEntry.timestamp);
    }
    const isEarlyExit = patienceTaxData !== null && patienceTaxData.maxMissedGain > 50;
    ...
    return {
        ...
        exitDetails: position.exits.length > 0
            ? {
...
export function calculateBehavioralMetrics(positions, options = {}) {
    ...
    const analyses = positions.map((p) => analyzePosition({
        position: p,
    ...
    const rawScore = breakdown.winRate.points +
...
export function computeConsistency(positionSizes) {
...
export function getArchetype(score, patienceTax, thresholds = DEFAULT_ARCHETYPE_THRESHOLDS) {
    if (score >= thresholds.ironPillar.minScore &&
        patienceTax <= thresholds.ironPillar.maxPatienceTax) {
        return "Iron Pillar";
    }
    if (score >= thresholds.profitPhantom.minScore &&
